#!/usr/bin/env python3
"""
Applique la passe C4.2 : complément du C4 initial pour les 71 entrées ISBE
orphelines liées à des concepts C2/C3 mais non couvertes par C4.

Stratégie identique à C4 initial (ne touche pas mot ni slug).
Log séparé pour traçabilité.
"""
import json
import sys
...
MAP_JSON = AUDIT_DIR / "isbe-c4-2-mot-map.json"
LOG_JSON = AUDIT_DIR / "isbe-c4-2-apply-log.json"


def check_backup():
    if not BACKUP_DIR.exists():
        print("ERREUR : aucun dossier work/backups/")
        return False
    backups = sorted(BACKUP_DIR.glob("dictionnaires-*.zip"))
    if not backups:
        print("ERREUR : aucun backup")
        return False
    latest = backups[-1]
...
def main():
    ...
    print(f"Mapping C4.2 : {len(mapped)} entrées sur {len(by_chunk)} chunks")
    print()
